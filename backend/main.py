...
def workbook_bytes(model):
    ...
    if ra:
        ws.append([])
        _rah = ws.max_row + 1
        ws.cell(_rah, 1, "PROGRAMME SCOPE ASSUMPTIONS").font = bold(9, "0E7490")
        ws.cell(_rah, 2, "Basis for cost estimate below").font = reg(8, "94A3B8")
        ws.append([])
        if ra.get("has_explicit_km") and ra.get("route_km"):
            cov_row("Route length", "{:,} km".format(ra["route_km"]))
        elif "km" not in str(model.get("title","")).lower():
            cov_row("Route length", "Derived from scope description — provide km for greater precision")
        if ra.get("stations"):
            cov_row("Station / interchange count", "~{} (estimated from route)".format(ra["stations"]))
        cov_row("Underground / tunnel", "{}% of route (HSR/urban standard)".format(ra.get("tunnel_pct",25)))
        cov_row("Elevated / viaduct", "{}% of route".format(ra.get("viaduct_pct",30)))
        cov_row("Surface / at-grade", "{}% of route".format(ra.get("surface_pct",45)))
        cov_row("Unit rate context", ra.get("topology_context","Major infrastructure programme"))

    # ── BENCHMARK REFERENCES ─────────────────────────────────────
    analogues = model.get("analogues", model.get("benchmark_comparison", []))
    if analogues:
        ws.append([])
        _bh = ws.max_row + 1
        ws.cell(_bh, 1, "BENCHMARK REFERENCES").font = bold(9, "0E7490")
        ws.cell(_bh, 2, "Named comparators used to calibrate this estimate").font = reg(8, "94A3B8")
        ws.append([])
        for ana in (analogues[:5] if isinstance(analogues, list) else []):
            if not isinstance(ana, dict): continue
            name = str(ana.get("name", ana.get("programme","")) or "")[:40]
            p50_str = str(ana.get("cost_p50","") or ana.get("cost","") or "")
            cost_growth = ana.get("cost_growth_pct", "")
            bench_line = name
            if p50_str: bench_line += " — {}".format(p50_str)
            if cost_growth: bench_line += " (+{}% cost growth)".format(cost_growth)
            ws.append(["  •  " + bench_line[:80]])
            ws.cell(ws.max_row, 1).font = reg(8, "334155")
        # Always mention named programme anchor if used
        anchor = model.get("auto_corrections", [])
        if anchor:
            for corr in anchor:
                if "rebased" in str(corr).lower() or "benchmark" in str(corr).lower():
                    ws.append(["  ⚡  Auto-corrected: " + str(corr)[:80]])
                    ws.cell(ws.max_row, 1).font = reg(8, "0E7490")
...
def export_qcra_qsra(model:Dict[str,Any]):
    try:
        _m2 = {k:v for k,v in (model or {}).items() if k not in ('all_schedule_levels','schedules_by_level','risk_register','risk_detail','cost_detail','self_challenge')}
        return stream(qcra_qsra_bytes(_m2),"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","CASEY_QCRA_QSRA.xlsx")
    except Exception as _ex:
        return stream_error("QCRA/QSRA: "+str(_ex)[:100])
...
def _get_unit_rate_display(line_cost_bn, total_p50_bn, unit, unit_label, prompt=''):
    ...
    if qty > 0 and line_cost_bn > 0:
        ...
        # Topology context for rail unit rates
        _ctx = ''
        _t = (prompt or '').lower()
        if unit in ('track-km', 'km', 'lane-km'):
            if any(w in _t for w in ['tunnel','underground','metro','bore']):
                _ctx = ' (blended surface/tunnel)'
            elif any(w in _t for w in ['viaduct','elevated','aerial']):
                _ctx = ' (surface/elevated)'
            elif any(w in _t for w in ['rail','hsr','transit']):
                _ctx = ' (blended all typologies)'
        if rate >= 1.0:
            return f'{money_bn(rate)}/{unit}{_ctx}'
        elif rate >= 0.001:
            rate_str = f'${round(rate*1000,0):.0f}M/{unit}'
            return rate_str + (_ctx if _ctx else '')
        else:
...
def _casey_final_cost_rows(mode: str, subsector: str, p50: float, scenario: str):
    ...
    for i, row in enumerate(template[:16], 1):
        cbs, desc, typ, basis, weight = row
        mid = raw_mids[i-1] * p50 / total_raw
        rows.append({
            'cbs': cbs,
            'description': desc,
            'type': typ,
            ...
            'p10_bn': round(mid * 0.82, 4),
            'p50_bn': round(mid, 4),
            'p90_bn': round(mid * 1.28, 4),
...
def build_model(prompt: str='', client: str='', class_level: int=3, schedule_level: int=4, scenario: str='base'):
    ...
    model = {
        ...
        'currency_symbol': _lc,
        'route_assumptions': _extract_route_assumptions(prompt, subsector),
