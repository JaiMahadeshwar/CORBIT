...
def _phase_packages_for_level(model, level):
    ...
    target={1:6,2:14,3:32,4:64,5:110}.get(level,32)
    if len(pairs) < target:
        idx=0
...
# Safer v63 wrappers with Body(default={}) fix 422 errors.
def all_zip_bytes(model):
    m=_payload_model(model)
    bio=BytesIO()
    with zipfile.ZipFile(bio,"w",zipfile.ZIP_DEFLATED) as z:
        z.writestr("CASEY_v63_Cost_Model.xlsx", cost_workbook_bytes(m))
        z.writestr("CASEY_v63_Risk_Register.xlsx", risk_workbook_bytes(m))
        z.writestr("CASEY_v63_Schedule.xer", patch_template_xer(m))
        z.writestr("CASEY_v63_Schedule_Levels.csv", schedule_csv_bytes(m))
        z.writestr("CASEY_v63_Model_Audit.json", model_json_bytes(m))
        z.writestr("README_v63.txt", "CASEY v63 True Universal Engine. Outputs are generated from the selected project type, Earth/Space mode, scenario, estimate class and schedule level. XER uses PRA-compatible ERMHDR 8.0 structure with dynamic CASEY WBS/tasks.\n")
    bio.seek(0); return bio.getvalue()
...
def install_v63(app):
    ...
    def workbook(payload: dict = Body(default={})):
        m=_payload_model(payload); return stream(cost_workbook_bytes(m),"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","CASEY_v63_Cost_Model.xlsx")
    def risks(payload: dict = Body(default={})):
        m=_payload_model(payload); return stream(risk_workbook_bytes(m),"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","CASEY_v63_Risk_Register.xlsx")
    def xer(payload: dict = Body(default={})):
        m=_payload_model(payload); return stream(patch_template_xer(m),"application/octet-stream","CASEY_v63_Schedule.xer")
    def csvexp(payload: dict = Body(default={})):
        m=_payload_model(payload); return stream(schedule_csv_bytes(m),"text/csv","CASEY_v63_Schedule_Levels.csv")
    def jsonexp(payload: dict = Body(default={})):
        m=_payload_model(payload); return stream(model_json_bytes(m),"application/json","CASEY_v63_Model_Audit.json")
    def allpack(payload: dict = Body(default={})):
        m=_payload_model(payload); return stream(all_zip_bytes(m),"application/zip","CASEY_v63_Output_Pack.zip")
    ...
    for p,e in [("/export/workbook",workbook),("/export/cost-model",workbook),("/export/risk-register",risks),("/export/xer",xer),("/export/schedule-csv",csvexp),("/export/json",jsonexp),("/export/all",allpack),("/export/full-pack",allpack),("/v63/export/all",allpack)]:
        replace_post(p,e)
