...
def _scenario_bucket_split_v132(scenario):
    key=str(scenario or "base").lower().replace(" ","_").replace("-","_")
    if key in ["faster","accelerated","fast"]: return {"Direct":0.70,"Indirect":0.18,"Reserve":0.12}
    if key in ["cheaper","low_cost","budget"]: return {"Direct":0.80,"Indirect":0.14,"Reserve":0.06}
    if key in ["lower_risk","low_risk","derisked"]: return {"Direct":0.66,"Indirect":0.16,"Reserve":0.18}
    if key in ["premium","resilient"]: return {"Direct":0.70,"Indirect":0.15,"Reserve":0.15}
    return {"Direct":0.74,"Indirect":0.14,"Reserve":0.12}

# Override costs so selected scenario clearly affects all class estimates and exports.
def normalize_cost_lines(model, scenario=None, cls=None):
    ...
    # m.cost_p50 is already the selected scenario P50. Do NOT multiply by scen['cost'] again.
    p50_total=as_float_money(m.get("cost_p50"))
    templates={
    ...
    split=_scenario_bucket_split_v132(scenario)
    type_sums={"Direct":0.0,"Indirect":0.0,"Reserve":0.0}
    for _,_,typ,p in lines: type_sums[typ]=type_sums.get(typ,0.0)+float(p)
    out=[]
    for cbs,desc,typ,p in lines:
        bucket_total=p50_total*split[typ]
        denom=type_sums.get(typ) or 1.0
        p50=bucket_total*(float(p)/denom)
        p10=p50*rule["low"]
        reserve_high = 1.55 if scenario in ["lower_risk","premium"] else 2.20 if scenario=="cheaper" else 1.85
        high_mult = reserve_high if typ=="Reserve" else rule["high"]
        p90=p50*high_mult; p80=p50+(p90-p50)*0.62
        out.append({"class":cls,"class_name":rule["name"],"maturity":rule["maturity"],"cbs":cbs,"description":desc,"type":typ,"p10_bn":round(p10,3),"p50_bn":round(p50,3),"p80_bn":round(p80,3),"p90_bn":round(p90,3),"scenario":scen["label"],"basis":f"{key.title()} sector CBS, {scen['label']} scenario bucket split {int(split[typ]*100)}% {typ}, {rule['name']} range."})
    drift=round(p50_total-sum(x["p50_bn"] for x in out),3)
    if abs(drift)>=0.001 and out:
        out[0]["p50_bn"]=round(out[0]["p50_bn"]+drift,3)
    return out
