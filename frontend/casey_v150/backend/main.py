...
def red_flags(risk, confidence, mode, subsector, scenario="base", p50=0.0, p80=0.0, sched_months=0):
    """Sector, scenario and quantitative red flags. Never returns generic non-answer."""
    s = subsector.lower()
    ...
    if not flags:
        flags.append(f"No sector-critical red flags at {confidence}% confidence — maintain assumptions log, stage-gate assurance and board decision register.")
...
def pptx_bytes(model: Dict[str, Any]) -> bytes:
    ...
    # ── SLIDE 8: Board attack kill-chain ────────────────────────────
    s = prs.slides.add_slide(blank); bg(s); amber_bar(s, top=True)
    heading(s, 'Board Attack Kill-Chain', 'The questions a serious investment committee will ask before approving capital.')
