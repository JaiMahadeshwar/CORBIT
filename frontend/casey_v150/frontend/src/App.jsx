...
function ProfessionalIntakeResult({ result, model }) {
  if (!result) return <div className="intakeEmpty"><b>No client file loaded.</b><span>Use the demo buttons below to show messy workbook challenge, XER schedule challenge, or risk-register challenge without needing a client file.</span></div>;
  let r = result;
  if (typeof r === 'string') {
    try { r = JSON.parse(r); } catch (_) { r = { review: r }; }
  }
  const file = r.filename || r.file || 'Messy_Client_Estimate_FINAL_v27.xlsx';
  const findings = asList(r.findings).length ? asList(r.findings) : asList(r.review).length ? asList(r.review) : [
    'CASEY detected an estimate-style input and mapped visible cost, schedule, risk and evidence signals into a board challenge model.',
    'Reported progress is being tested against the governing readiness constraint instead of accepted as programme truth.',
    'Reserve posture is being reconciled to P80/P90 exposure, not headline median optimism.'
  ];
  const flags = asList(r.red_flags || r.flags).length ? asList(r.red_flags || r.flags) : [
    'Missing evidence closure for the governing constraint.',
    'No named owner for the top board-risk driver.',
    'Contingency and schedule float are not yet proven usable.'
  ];
  const next = asList(r.next_steps || r.next_action).length ? asList(r.next_steps || r.next_action) : [
    'Request source workbook, XER schedule and risk register as a single evidence bundle.',
    'Run the scenario re-test before board approval.',
    'Export the challenged pack after the evidence gate is closed.'
  ];
  const gate = model?.confidence_pct < 55 ? 'DO NOT APPROVE WITHOUT MORE EVIDENCE' : 'BOARD CHALLENGE LIKELY';
  return <div className="intakeResult proChallenge">
    <div className="intakeHeader"><span>LIVE CLIENT INTAKE CHALLENGE</span><b>{file}</b><em>{r.size_bytes ? `${Math.round(r.size_bytes/1024)} KB normalised` : 'messy-file demo mode'}</em></div>
    <div className="challengeVerdict"><span>CASEY VERDICT</span><b>{gate}</b><p>This is not a file upload receipt. CASEY is challenging the client position and asking what must be proven before approval.</p></div>
    <div className="intakeMetrics">
      <div><span>Schema detector</span><b>{r.schema_confidence || 'Auto-mapped'}</b></div>
      <div><span>Governing constraint</span><b>{model?.primary_constraint || 'interface control + evidence closure'}</b></div>
      <div><span>Board test</span><b>P50 / P80 / P90 reconciliation</b></div>
    </div>
    <h3>Challenge findings</h3>{findings.map((x,i)=><div className="reason" key={i}><span>{i+1}</span>{x}</div>)}
    <h3>Board red flags</h3>{flags.map((x,i)=><div className="reason danger" key={i}><span>!</span>{x}</div>)}
    <h3>Traceability chain</h3>{[
      'Uploaded or sample source classified as estimate / schedule / risk evidence.',
      'Fields and messy labels mapped to cost packages, schedule drivers, risk owners and evidence gates.',
      'Scenario state re-scored for cost, duration, confidence, reserve and board posture.',
      'Exports are stamped from the challenged model, not a separate narrative.'
    ].map((x,i)=><div className="traceStep" key={i}><span>{i+1}</span><b>{x}</b></div>)}
    <h3>Next action</h3>{next.map((x,i)=><div className="reason" key={i}><span>{i+1}</span>{x}</div>)}
  </div>
}
...
function HolyGrailRuntime({ model, scenario, generate, runShock }) {
  const controls = [
    ['signalling_slip','What if signalling slips 4 months?','Propagate schedule tail, confidence and board attack chain'],
    ['procurement_gap','What if procurement evidence is weak?','Lower confidence, increase P80/P90 and expose owner actions'],
    ['reserve_cut','What if contingency is cut 10%?','Reprice reserve posture and governance warning'],
    ['operator_delay','What if operator acceptance moves late?','Move governing chain to commissioning readiness']
  ];
  return <section className="layout two runtimePanel">
    <Card><h2>Holy Grail Runtime Controls</h2><p className="big">These buttons mutate the active model instead of showing a pre-written paragraph. Use them live in the demo: click one, then watch cost, schedule, confidence, risk posture and board language change.</p><div className="runtimeButtons">{controls.map(([id,title,sub])=><button key={id} onClick={()=>runShock(id)}><Zap size={16}/><b>{title}</b><span>{sub}</span></button>)}</div><h3>Scenario re-run engine</h3><div className="runtimeScenarioRow">{scenarios.map(s=><button key={s} className={s===scenario?'active':''} onClick={()=>generate(s, model?.prompt, model)}>{s.replace('_',' ')}</button>)}</div></Card>
    <Card><h2>Current Source of Truth</h2>{[
      ['Scenario', model?.scenario_label || scenario],
      ['P50 cost', moneyLocal(model?.cost_p50_bn || model?.p50_cost_bn || 0)],
      ['Schedule', `${model?.schedule_months || model?.duration_months || '—'} months`],
      ['Confidence', `${model?.confidence_pct ?? '—'}%`],
      ['Governing chain', (model?.causal_chain || model?.governing_chain || []).join(' → ') || 'Constraint → evidence → confidence → board decision'],
      ['Decision posture', model?.decision_posture || model?.confidence_label || 'Board challenge']
    ].map(([k,v],i)=><div className="reason" key={k}><span>{i+1}</span><b>{k}</b><br/>{v}</div>)}<h3>Explainability proof</h3><div className="traceStep"><span>1</span><b>Every click updates canonical scenarioState.</b></div><div className="traceStep"><span>2</span><b>Derived outputs refresh from the same model state.</b></div><div className="traceStep"><span>3</span><b>Exports use the mutated model payload.</b></div></Card>
  </section>
}
...
function parseMoneyLocal(v) {
  ...
  async function ask(overrideQ) {
    ...
    if (ql.includes('fail') && (ql.includes('programme') || ql.includes('blame') || ql.includes('publicly'))) {
      instant = ifFails ? ifFails ? ['**IF THIS PROGRAMME FAILS**','',ifFails,'','This is the governing failure mode for this programme type. The board must close this risk before capital is committed.'].join('\n') : null;
    } else if (ql.includes('traditional') || ql.includes('controls vs') || ql.includes('what would a traditional')) {
      if (tvc.traditional && tvc.casey) {
        instant = ['**TRADITIONAL CONTROLS vs CASEY**','','Traditional view: ' + (tvc.traditional||'Civil progress and costs appear on track.'),'','**What CASEY reads underneath:**',''+( tvc.casey||'Governing constraint not visible in headline progress.'),'','P80 tail: ' + (tvc.tail_pct||25) + '% above P50.','','The gap between these two readings is the product.'].join('\n');
      }
      ...
      if (chain) instant = ['**REAL GOVERNING CAUSAL CHAIN**','',chain,'','This chain drives cost, schedule and confidence. If any link is weak, the model shifts right.'].join('\n');
      ...
      instant = ['**ASSUMPTIONS THAT COLLAPSE CONFIDENCE FIRST**','', ...bqs.map((q,i)=>`${i+1}. ${q}`),'','Closing evidence on these is the fastest route to board approval.'].join('\n');
      ...
      instant = ['**THE ONE INTERVENTION THAT CHANGES CONFIDENCE FASTEST**','',`Close the evidence gap on: ${constraint}`,''  ,'This means: named owner + named trigger + quantified residual + documented closure date.','That single action moves from "is this approvable?" to "what is the decision?".'].join('\n');
    } else if (ql.includes('external assurance') || ql.includes('assurance reviewer')) {
      const attacks = model?.board_attack_simulation || [];
      instant = ['**WHAT AN EXTERNAL ASSURANCE REVIEWER ATTACKS FIRST**','','1. Is the P50 defensible against QCRA P80 — or does it mask the downside?','2. Is reserve linked to named risks — or is it a flat percentage?','3. Who owns the governing constraint, and what is the evidence closure date?','4. Is schedule float operationally usable — or nominal and already consumed?','5. What evidence package retires the approval blocker before the next gate?',''  , ...(attacks.slice(0,3).map((a,i)=>`${i+6}. ${a}`))].join('\n');
    } else if (ql.includes('destroy board confidence') || ql.includes('destroy confidence')) {
      instant = ['**WHAT WOULD DESTROY BOARD CONFIDENCE FASTEST**','','1. A cost increase that was already known but not disclosed.','2. A schedule slip that reveals float was never real.','3. A risk with no named owner and no mitigation evidence.','4. A commissioning failure cascading into operator rejection.','5. A procurement gap forcing sole-source at programme peak.','',`For this programme: ${model?.executive_shock_insight || 'confidence sits in integration and evidence maturity, not visible civil progress.'}`].join('\n');
    } else if (ql.includes('management probably hiding') || ql.includes('looks green')) {
      const shock = model?.executive_shock_insight;
      const flags = (model?.red_flags || []).slice(0,3);
      instant = ['**WHAT MANAGEMENT IS PROBABLY HIDING**','', shock || 'The governing constraint is not in the headlines.','', ...(flags.length ? ['**Red flags CASEY identified:**', ...flags.map((f,i)=>`${i+1}. ${f}`)] : []),'','Traditional project controls reports show numbers. CASEY shows what the numbers are trying to hide.'].join('\n');
    }
    if (instant) {
  ...
  function runShock(kind) {
    ...
    const shock = {
      ...
      reserve_cut: { months: 0, cost: -0.03, conf: -12, note: 'Reserve cut improves headline cost but consumes downside protection and weakens approval posture.' },
    ...
    if (!shock) return;
    const nextCost = Math.max(.1, (model.cost_p50_bn || model.p50_cost_bn || 1) * (1 + shock.cost));
    const nextMonths = Math.max(1, Math.round((model.schedule_months || model.duration_months || 1) + shock.months));
    const nextConf = Math.max(5, Math.min(95, Math.round((model.confidence_pct || 50) + shock.conf)));
    setModel({ ...model, cost_p50_bn: nextCost, p50_cost_bn: nextCost, schedule_months: nextMonths, duration_months: nextMonths, confidence_pct: nextConf, confidence_label: nextConf < 45 ? 'Do not approve without more evidence' : 'Board challenge likely', executive_shock_insight: shock.note, board_briefing: [shock.note, `Runtime mutation applied: ${kind.replaceAll('_',' ')}.`, `Updated position: ${moneyLocal(nextCost)} P50, ${nextMonths} months, ${nextConf}% confidence.`, 'Exports are now stamped from this changed model state.'], casey_thinking: `CASEY runtime mutation: ${shock.note} The model recalculated cost, schedule, confidence, reserve posture and board challenge language from the same source of truth.`, scenario_trade: shock.note, scenario_comparison_vs_base: { ...(model.scenario_comparison_vs_base || {}), plain_english: shock.note }, last_runtime_event: kind });
    setTab('overview');
  }
  ...
  return <div className="app v50EliteApp">
    ...
    <main className={model ? 'v50Console' : 'v50Console emptyConsole'}>
      ...
      {model && <>
        ...
        <nav className="tabs">{[['overview','Overview'],['compare','Scenarios'],['delta','Scenario Intel'],['causal','Causal OS'],['cost','Cost'],['schedule','Schedule'],['risk','Risk'],['monte','QCRA/QSRA'],['outputs','Outputs'],['assurance','Assurance'],['runtime','Holy Grail'],['advisor','Advisor'],['method','Methodology'],['pricing','Pricing']].map(x => <button key={x[0]} className={tab===x[0]?'active':''} onClick={() => setTab(x[0])}>{x[1]}</button>)}</nav>
        {tab === 'overview' && <>
        ...
        {tab === 'advisor' && <section className="layout two advisorElite challengeRoom"><Card><h2>CASEY Board Attack Console</h2><p className="advisorIntro">No typing needed. Click an attack button and CASEY answers like an assurance reviewer challenging the current board position.</p><div className="advisorPrompts bigButtons">{['What is the board not seeing?','What would a traditional cost consultant say that CASEY challenges?','What evidence is missing before this becomes board-approvable?','What is the real governing chain?','Which assumptions collapse confidence first?','What is the board really deciding?','If this programme fails, what will be blamed publicly?','Give me CASEY POSITION.','What is management probably hiding?','What would destroy board confidence fastest?','What problem looks green but is actually red?','Show Traditional Controls vs CASEY.','What is the one intervention that changes confidence fastest?','What would an external assurance reviewer attack first?'].map(x=><button key={x} data-question={x} onClick={()=>ask(x)}><Brain size={14}/>{x}</button>)}</div><div className="chatBox boardInterrogation">{chat.length ? chat.map((m,i)=><div key={i} className={`msg ${m.role}`}>{(() => {
  const txt = String(m.text || '');
  return <div className="msgBody">{txt.split('\n').filter(l => l).map((line, li) => {
    const parts = line.split(/\*\*([^*]+)\*\*/g);
    return <p key={li} style={{margin:'3px 0',lineHeight:'1.55'}}>{parts.map((p, pi) => pi % 2 === 1 ? <strong key={pi} style={{color:'#8df7ff'}}>{p}</strong> : p)}</p>;
  })}</div>;
})()}</div>) : <div className="msg assistant"><b>Board attack ready.</b><br/>Click any challenge above. CASEY will answer against the active scenario, not as a generic chatbot.</div>}</div><div className="ask"><input value={chatQ} onChange={e=>setChatQ(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')ask()}} placeholder="Optional custom board question..."/><button onClick={() => ask(chatQ)}>Ask</button></div></Card><Card><h2>Live Client Challenge Room</h2><p className="advisorIntro">This is the upload/challenge area to show Turner & Townsend style buyers. Use the three buttons below even when you have no client file. The output is a professional challenge pack, not JSON.</p><div className="challengeHero"><span>CASEY INTAKE NORMALISATION ENGINE</span><b>Messy file → schema detection → WBS/CBS inference → evidence gaps → board attack → export-ready challenge</b></div><div className="challengeButtons pro"><button onClick={()=>setUploadResult({filename:'Messy_Client_Estimate_FINAL_v27.xlsx', schema_confidence:'XLSX auto-mapped from messy workbook', findings:['Detected inconsistent estimate structure and normalised visible rows into cost packages, basis statements, contingency signals and missing evidence gates.','CASEY does not reject the headline number; it blocks board approval until reserve and basis are reconciled to P80/P90 exposure.','The governing issue is not the civil estimate. It is whether commissioning, interface and procurement risks have named owners and evidence closure.'], red_flags:['Possible duplicated / unclear package basis in the estimate workbook.','Contingency appears median-led unless P80/P90 support is supplied.','No clean owner-action-evidence chain for the constraint controlling confidence.'], next_steps:['Request source workbook plus WBS/CBS map.','Require P80/P90 reconciliation before approving the number.','Re-run scenario after evidence closure and export the challenged board pack.']})}><FileSpreadsheet/><b>Challenge messy cost workbook</b><span>Broken tabs, renamed columns, missing basis, hidden contingency.</span></button><button onClick={()=>setUploadResult({filename:'Client_Schedule_Export_FINAL.xer', schema_confidence:'XER schedule logic mapped', findings:['Activity logic challenged for weak predecessor/successor evidence, open ends and artificial float.','Critical path is treated as a hypothesis until handover, commissioning and operator acceptance are proven.','A faster date is not accepted unless recovery crews, possessions and permits are funded.'], red_flags:['Open ends / weak logic ties may understate delivery tail risk.','Float may not be operationally usable.','Board needs QSRA P80/P90 date before approval.'], next_steps:['Upload XER with calendars and coding fields.','Run QSRA refresh and compare P50/P80/P90 against management date.','Assign named owner to the governing schedule constraint.']})}><Workflow/><b>Challenge XER schedule</b><span>Logic ties, critical path, float, QSRA tail and recovery realism.</span></button><button onClick={()=>setUploadResult({filename:'Risk_Register_Client_Draft.xlsx', schema_confidence:'Risk register schema mapped', findings:['Risk rows mapped into cause, event, impact, owner, mitigation and residual exposure.','CASEY challenges rows with no trigger, no owner, no quantified residual exposure or no evidence closure.','P80/P90 exposure is linked back to the risks that actually move board confidence.'], red_flags:['Mitigations read like action phrases, not evidence closure.','Owner accountability is incomplete.','Residual exposure is not reconciled to reserve.'], next_steps:['Require named owner, trigger, mitigation and residual exposure on every top risk.','Export risk register after QCRA/QSRA alignment.','Use board attack questions before approval.']})}><ShieldAlert/><b>Challenge risk register</b><span>Cause-event-impact-owner-mitigation-residual exposure audit.</span></button></div><h3>Upload real file</h3><label className="upload proUpload"><Upload size={18}/> Upload estimate / XER / risk workbook<input type="file" onChange={upload}/></label><ProfessionalIntakeResult result={uploadResult} model={model}/></Card></section>}
