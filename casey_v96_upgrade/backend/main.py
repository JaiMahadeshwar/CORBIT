...
def scenario_cascade_v95(model:Dict[str,Any], scenario:str) -> Dict[str,Any]:
    ...
    model["scenario_comparison_vs_base"]={
        "base":{"cost_p50":base_cost_money,"schedule_months":base_schedule_months,"confidence_pct":base_conf_pct,"risk":base.get("risk","Medium-High")},
        ...
        "plain_english": f"Compared with Base, {profile['label']} is {_bn_to_money_hs(abs(delta_cost))} {'more expensive' if delta_cost>0 else 'cheaper' if delta_cost<0 else 'unchanged'}, {abs(delta_months)} months {'faster' if delta_months<0 else 'slower' if delta_months>0 else 'unchanged'}, and {abs(delta_conf)} confidence points {'higher' if delta_conf>0 else 'lower' if delta_conf<0 else 'unchanged'}."
    }
...
def build_model(prompt:str, client:str="", class_level:int=3, schedule_level:int=3, scenario:str="base"):
    ...
    cm,sm,rm,conf_adj,scenario_label,scenario_why=scenario_params("base")  # scenario cascade applies scenario after base model is built
...
def build_model_core_no_compare(prompt,client,cls,level,scenario):
    title,mode,subsector,base_cost,base_months=detect_sector(prompt); location,loc_mult=location_factor(prompt); scale,scale_mult,scale_months=scale_factor(prompt); comp_mult,comp_months,drivers=complexity(prompt); cm,sm,rm,conf_adj,scenario_label,scenario_why=scenario_params("base")  # scenario cascade applies scenario after base model is built
...
SCENARIO_PROFILES_V95_PLUS = {
    ...
    "lower_risk": {
        "label":"Lower Risk","cost":1.12,"schedule":1.16,"confidence":14,"risk":"Medium",
        ...
        "won":"Reduced P80/P90 exposure and stronger board approval defensibility.",
        "lost":"Earlier revenue date and lean capital posture."
    },
    "premium": {
        "label":"Premium","cost":1.28,"schedule":0.96,"confidence":20,"risk":"Medium-Low",
        "reserve_factor":1.65,"direct_factor":1.22,"indirect_factor":1.24,
        "curve":"premium","risk_lift":-16,
        "trade":"You bought resilience, redundancy and strategic optionality.",
        "won":"Higher operational resilience, stronger procurement certainty and better downside protection.",
        "lost":"Lowest-capex authorization case."
    },
}
...
def _fam_insight_pack_hs(model):
    ...
    if fam=="data_centre":
        return {
            "base":"Power availability, energisation and IST concurrency are more likely to govern delivery than shell construction productivity.",
            "faster":"Acceleration is forcing grid energisation, cooling readiness and IST into a compressed window. The data halls may be built before the power system can be trusted.",
            "cheaper":"The cheaper case likely saves capex by reducing redundancy, commissioning margin and energy resilience — exactly where uptime risk lives.",
            "lower_risk":"The lower-risk case buys power certainty, IST float and operational resilience before capacity is handed to customers.",
            "premium":"Premium delivery buys redundancy, power optionality and stronger uptime credibility."
        }
    if fam=="semiconductor":
        return {
...
def _curve_hs(cost, months, scenario):
    ...
    profiles={
        ...
        "faster": {
            ...
            "meaning":"Faster case: median date improves, but P80/P90 tail expands because recovery float is consumed."
        },
        "cheaper": {
        ...
        "lower_risk": {
            ...
            "meaning":"Lower-risk case: higher median, tighter tail and more board-defensible certainty."
        },
        "premium": {
            ...
            "meaning":"Premium case: capex is higher, but the downside tail is controlled through resilience and procurement priority."
        },
    }
