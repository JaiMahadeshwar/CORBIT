// CASEY V168.3 CONTROLORBIT PRODUCTION RENDER HARDENED - v20260525
...
function safeRender(value) {
  ...
  if (typeof value === 'object') {
    // Common CASEY narrative object: render as executive prose, not JSON/code.
    const orderedKeys = ['headline', 'summary', 'decision_rule', 'decisionRule', 'primary_constraint', 'constraint', 'plain_english', 'meaning', 'label', 'value', 'note'];
    const parts = orderedKeys
      .filter((k) => value[k] !== undefined && value[k] !== null)
      .map((k) => safeRender(value[k]))
      .filter(Boolean);
    if (parts.length) return parts.join(' · ');
    return Object.values(value).map(safeRender).filter(Boolean).join(' · ');
  }
  return String(value || '');
}
    ...
    <div className="intakeResult proChallenge professionalReview">
      <div className="challengeFileBar proFileBar">
        <div className="challengeFileInfo">
          <span className="challengeFileTag">Independent source-file challenge</span>
        ...
        <div className="challengeLiveTag">CLIENT-SIDE REVIEW</div>
      </div>
      ...
      <div className="challengeMetricRow professionalMetrics">
        <div className="cm hot"><span>Submitted / inferred P50</span><b>{challengeP50}</b></div>
        <div className="cm"><span>Challenge P80 position</span><b style={{color:'#ff9940'}}>{challengeP80}</b></div>
        <div className="cm"><span>Stress / P90 position</span><b style={{color:'#ff6b7d'}}>{challengeP90}</b></div>
        <div className="cm"><span>Reliance confidence</span><b style={{color: confPct<60?'#f7d774':'#8df7ff'}}>{confPct}%</b></div>
      </div>
      ...
      <div className="challengeSection">
        <div className="challengeSectionHead attack"><span className="csh-num">Q</span> Board assurance questions</div>
...
class CaseyErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.error) {
      return <div className="app v50EliteApp"><main className="v50Console"><section className="layout one"><div className="card shockCard"><h2>CASEY runtime guard</h2><p>A production guard caught an old object-render response. Refresh once. If it repeats, redeploy this fixed build so advisory objects are rendered as professional text.</p><pre>{safeRender(this.state.error?.message || this.state.error)}</pre></div></section></main></div>;
    }
...
function exportAuditSpine(model, direct, indirect, reserves, reconcileCheck) {
  ...
  return [
    ...
    { label:'Competitive threat', value:'High', detail:'Replaces manual slide-production with auditable scenario propagation and board assurance logic.' },
  ...
  return <>
    <section className="layout two incumbentPressure">
      ...
      <Card className="threatCard">
        ...
        <h3>Audit spine</h3>
        {spine.map((x,i)=><div className={`auditSpine ${x.value==='PASS'?'pass':''}`} key={safeRender(x.label)}><span>{i+1}</span><b>{safeRender(x.label)}: {safeRender(x.value)}</b><em>{x.detail}</em></div>)}
      </Card>
    </section>
    {ifFails && (
...
function parseMoneyLocal(v) {
  ...
  async function ask(overrideQ) {
    ...
    if (!q) return;
    if (!model) {
      setChat(x => [...x, {role:'user',text:q}, {role:'assistant',text:'**No project loaded yet**\n\nGenerate a project first, then come back to challenge it. Board assurance answers are grounded in the live model.'}]);
      return;
    }
    setChatQ('');
      ...
      <nav className="tabs">{[['overview','Overview'],['compare','Scenarios'],['delta','Scenario Intel'],['causal','Causal OS'],['cost','Cost'],['schedule','Schedule'],['risk','Risk'],['monte','QCRA/QSRA'],['outputs','Outputs'],['assurance','Assurance'],['runtime','Live Stress Test'],['advisor','Advisor'],['method','Methodology'],['pricing','Pricing']].map(x => <button key={x[0]} className={tab===x[0]?'active':''} onClick={() => setTab(x[0])}>{x[1]}</button>)}</nav>
        {tab === 'overview' && <>
          ...
          <section className="layout two eliteLayer">
            ...
            <Card><h2>Likely board questions</h2>{boardQuestions(model).slice(0,6).map((x,i)=><div className="reason" key={safeRender(x)}><span>{i+1}</span>{safeRender(x)}</div>)}<h3>CASEY final position</h3><p className="caseyThinking finalPosition">{finalPosition(model)}</p></Card>
          </section>
        ...
        {tab === 'delta' && <section className="layout two">
          <Card><h2>Strategic Delta Intelligence</h2><p>What changed because this scenario was selected.</p>
            {(model.scenario_delta_intelligence || []).map((x,i)=><div className="reason" key={i}><span>{i+1}</span><b>{safeRender(x.label)}: {safeRender(x.value)}</b><br/>{safeRender(x.meaning)}</div>)}
          </Card>
        ...
        {tab === 'delta' && <section className="layout two">
          <Card className="shockCard"><h2>Scenario Consequence vs Base</h2><p>{model.scenario_trade || 'Scenario trade-off analysis.'}</p>
            {(model.scenario_delta_intelligence || []).map((x,i)=><div className="reason deltaReason" key={i}><span>{i+1}</span><b>{safeRender(x.label)}: {safeRender(x.value)}</b><br/>{safeRender(x.meaning)}</div>)}
          </Card>
